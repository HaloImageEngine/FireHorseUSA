...
export class LoginComponent {
  ...
  private doPostLoginUI(aliasCookie: string, emailCookie: string | null, userIdVal: string | number) {
    console.log('=== doPostLoginUI ===');
    console.log('Received userIdVal:', userIdVal, 'type:', typeof userIdVal);

    // Convert userId to number
    const userIdNum = Number(userIdVal) || 0;
    console.log('Converted to userIdNum:', userIdNum, 'type:', typeof userIdNum);
      ...
      try {
        ...
        if (stored) {
          ...
          if (data.email && data.alias === aliasCookie) {
            ...
            console.log('Preserved email from localStorage:', finalEmail);
          }
        }
        ...
        if (!finalEmail) {
          ...
          if (cookieStr) {
            ...
            if (cookieData.email && cookieData.alias === aliasCookie) {
              ...
              console.log('Preserved email from cookie:', finalEmail);
            }
          }
        }
      } catch (err) {
        console.log('Error trying to preserve email:', err);
      }
    }

    console.log('Setting cookies with alias:', aliasCookie, 'email:', finalEmail, 'userId:', userIdNum);
    console.log('=== END doPostLoginUI ===');
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      if (!data || data.ok !== true || !data.userId) {
        ...
        console.log('Login failed - API response:', JSON.stringify(data, null, 2));
        return;
      }
      ...
      if (status && (status === 404 || status !== 200)) {
        ...
        console.log('Error details:', JSON.stringify(err?.error || err, null, 2));
